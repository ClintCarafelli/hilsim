...
#---------------------------------------------------------------------------------------
def build_actuators(
    actuator_config: dict, world_state: WorldState, controller: Controls
) -> dict:
    """Create dictonary of instances of each actuator"""
    actuators = {}
    for name, config in actuator_config.items():
        driver_name = config["settings"]["driver"]
        driver_class = ActuatorBase.registry[driver_name]
        actuators[name] = driver_class(config, world_state, controller)
