...
class WorldState():
    ...
    def __init__(self, config):
        self.config = config
        self._memory = {name: var["initial"] for name, var in config["variables"].items()}
    ...
    def update_state(self): 
        for variable in self.config["variables"].keys(): 
            self.state[variable] = self._memory[variable] + sum([c for c in self._contributions[variable].values()])
