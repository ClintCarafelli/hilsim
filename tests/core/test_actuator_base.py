...
# Mock away the controller and world_state for unit tests
CONTROLLER = MagicMock()
WORLD_STATE = MagicMock()

# Create child class to help test the __init_subclass__ method
...
@pytest.fixture
def actuator_base(actuator_config: dict):
    ...
    with patch.object(ActuatorBase, "_discover_effects", return_value="fake_method"):
        yield ActuatorBase(actuator_config["actuator_1"], WORLD_STATE, CONTROLLER)


@pytest.fixture
def child_class(actuator_config: dict) -> ChildClass:
    """Create instance of a child class that inherits from actuator base"""
    return ChildClass(actuator_config["actuator_1"], WORLD_STATE, CONTROLLER)
...
def test_send_command_no_advanced_connection(child_class: ChildClass) -> None:
    ...
    with (
        ...
        child_class.send_command("state_2", False)
        child_class.build_command.assert_called_once_with("state_2")
        child_class._controller.send.assert_called_once_with("Device1", "0,1,1")
        child_class._update_commanded_state.assert_called_once_with(
            {"confirmed": True}, "state_2"
        )
...
def test_update_commanded_state_not_confirmed(actuator_base: ActuatorBase) -> None:
    """Test updating the commanded state when confirmed"""
    actuator_base._update_commanded_state({"confirmed": False}, "state_2")
    assert actuator_base.commanded_state == "state_1"
    assert actuator_base._transition_state == "settled"
...
def test_newly_stuck(actuator_base: ActuatorBase) -> None:
    ...
    with patch("hilsim.core.actuators.logger") as mock_logger:
...
def test_transition_state_is_settled(actuator_base: ActuatorBase) -> None:
    ...
    assert actuator_base._stuck_counter == 0
    assert actuator_base.commanded_state == "state_1"
    assert actuator_base.hardware_state == "state_1"
...
def test_sample_latency_normal_distribution(
    ...
    with patch("hilsim.core.actuators.gauss", return_value=latency) as mock_gauss:
...
def test_sample_latency_uniform_distribution(actuator_base: ActuatorBase) -> None:
    ...
    with patch("hilsim.core.actuators.uniform", return_value=0.0025) as mock_uniform:
        result = actuator_base._sample_latency()
...
def test_sample_stuck(
    ...
    """test _sample_stuck"""
    with patch("hilsim.core.actuators.random", return_value=random_return_val):
...
def test_discover_effects(child_class: ChildClass) -> None:
    ...
    assert len(em_dict) == 2
    should_be_keys = {"variable_1", "variable_2"}
...
def test_apply_effects(child_class: ChildClass) -> None:
    ...
    assert child_class.world_state.apply_delta.call_count == 1
