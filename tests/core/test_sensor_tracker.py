...
SENSOR1_READING_1 = Reading(10, "m")
SENSOR1_READING_2 = Reading(20, "kg")
SENSOR2_READING_1 = Reading(2, "m/s")
SENSOR2_READING_2 = Reading(-0.5, "m/s^2")
SENSOR_LIST_1 = {"variable_1": SENSOR1_READING_1, "variable_2": SENSOR1_READING_2}
SENSOR_LIST_2 = {"variable_3": SENSOR2_READING_1, "variable_4": SENSOR2_READING_2}
NOM_RESULTS = {"sensor_1": SENSOR_LIST_1, "sensor_2": SENSOR_LIST_2}

# Define some "failed readings (e.g. have a value of None)"
SENSOR1_READING_F1 = Reading(None, "m")
SENSOR1_READING_F2 = Reading(None, "kg")
SENSOR_LIST_F1 = {"variable_1": SENSOR1_READING_F1, "variable_2": SENSOR1_READING_F2}
FAIL_RESULTS = {"sensor_1": SENSOR_LIST_F1, "sensor_2": SENSOR_LIST_2}


@pytest.fixture
def sensor_tracker(sensor_config) -> SensorTracker:
    """Create an instance of a sensor tracker"""
    return SensorTracker(sensor_config)

@pytest.fixture(autouse=True)
def mock_logger() -> Generator[MagicMock, None, None]:
    """Create a mock logger to be used throughout"""
    with patch("hilsim.core.sensor_tracker.logger") as m:
        yield m
...
def test_setup(sensor_tracker: SensorTracker) -> None:
    ...
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 0, "sensor_2": 0}
...
def test_track_failed_readings_transient(
    ...
    assert results == {"i2c_cycle": False, "power_cycle": False}
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 1, "sensor_2": 0}
    mock_logger.error.assert_called()

    results = sensor_tracker.track(FAIL_RESULTS)
    assert results == {"i2c_cycle": False, "power_cycle": False}
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 2, "sensor_2": 0}
    mock_logger.error.assert_called()

    results = sensor_tracker.track(FAIL_RESULTS)
    assert results == {"i2c_cycle": False, "power_cycle": False}
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 3, "sensor_2": 0}
    mock_logger.error.assert_called()
...
def test_track_failed_readings_software_cycle(
    ...
    sensor_tracker.sensor_errors = {"sensor_1": 3, "sensor_2": 0}
    results = sensor_tracker.track(FAIL_RESULTS)
    assert results == {"i2c_cycle": True, "power_cycle": False}
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 4, "sensor_2": 0}
    mock_logger.error.assert_called()
    ...
    assert results == {"i2c_cycle": True, "power_cycle": False}
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 5, "sensor_2": 0}
    mock_logger.error.assert_called()
    mock_logger.info.assert_called()
...
def test_track_failed_readings_power_cycle(
    ...
    sensor_tracker.sensor_errors = {"sensor_1": 5, "sensor_2": 0}
    results = sensor_tracker.track(FAIL_RESULTS)
    assert results == {"i2c_cycle": False, "power_cycle": True}
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 6, "sensor_2": 0}
    mock_logger.error.assert_called()
    mock_logger.info.assert_called()
    ...
    assert results == {"i2c_cycle": False, "power_cycle": True}
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 7, "sensor_2": 0}
    mock_logger.error.assert_called()
    mock_logger.info.assert_called()
...
def test_track_dead_sensor_decleration(
    ...
    sensor_tracker.sensor_errors = {"sensor_1": 7, "sensor_2": 0}
    results = sensor_tracker.track(FAIL_RESULTS)
    assert sensor_tracker.sensor_errors == {"sensor_1": 0, "sensor_2": 0}
    assert results == {"i2c_cycle": False, "power_cycle": False}
    assert sensor_tracker.dead_sensors == {"sensor_1": True, "sensor_2": False}
    mock_logger.error.assert_called()
...
def test_track_dead_sensor_no_mitigation(sensor_tracker: SensorTracker) -> None:
    ...
    sensor_tracker.dead_sensors = {"sensor_1": True, "sensor_2": False}
    results = sensor_tracker.track(FAIL_RESULTS)
    assert results == {"i2c_cycle": False, "power_cycle": False}
    assert sensor_tracker.dead_sensors == {"sensor_1": True, "sensor_2": False}
    assert sensor_tracker.sensor_errors == {"sensor_1": 0, "sensor_2": 0}
...
def test_track_reset_error_count(sensor_tracker: SensorTracker) -> None:
    ...
    sensor_tracker.sensor_errors = {"sensor_1": 2, "sensor_2": 5}
    results = sensor_tracker.track(NOM_RESULTS)
    assert sensor_tracker.sensor_errors == {"sensor_1": 0, "sensor_2": 0}
    assert results == {"i2c_cycle": False, "power_cycle": False}
    assert sensor_tracker.dead_sensors == {"sensor_1": False, "sensor_2": False}
